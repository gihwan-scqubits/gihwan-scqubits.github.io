...
Where does that inductance come from? Zero resistance is not zero inductance. Any current stores energy in its magnetic field, which is the geometric inductance of the ground path, and a superconductor has it like any other conductor. A supercurrent is also carried by Cooper pairs with mass, and accelerating them costs energy: that is the kinetic inductance, set by the London penetration depth $$\lambda$$ and larger for thinner or dirtier films [6, 7]. For a wide aluminium ground plane both are picohenries, against nanohenries for a junction. Tiny, but not zero, and the contradiction above is exactly what happens when you set it to zero: a superconducting ring obeys fluxoid quantization, and its inductance times a screening current makes up the difference between the applied flux and an integer number of flux quanta. Just as important for the circuit model, the inductance adds a node. In Fig. c the two ground paths are shorted into one node, so two loop constraints act on a single node flux, which is one constraint too many. With inductance, the point where the ground paths meet junction 1 becomes its own node, $$\phi'$$, and that extra degree of freedom is exactly what the equations were missing.
